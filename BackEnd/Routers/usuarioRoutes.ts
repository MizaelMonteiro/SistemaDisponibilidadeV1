...
router.put("/:id", (req, res) => {
    ...
    const usuario = usuariosController.atualizar(usuarioAtualizado,id)
    usuario
        .then((resulUsuarioAtualizado =>res.status(200).json(resulUsuarioAtualizado)))
        .catch((error)=> res.status(400).json(error.message))
...
router.delete("/:id", (req, res) => {
    ...
    usuario
        .then((resultadoDeletar =>res.status(200).json(resultadoDeletar)))
        .catch((error)=> res.status(400).json(error.message))
});
